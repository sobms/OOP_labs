	...
	if (a->get_name() == b->get_name()) {
		if (a->get_type() == b->get_type()) {
			if (typeid(*a) == typeid(LM::Crossing_station)) {
				...
				std::list<std::string> lst1 = c_a->get_transition_lines();
				std::list<std::string> lst2 = c_b->get_transition_lines();
				std::list<std::string>::iterator it1;
				std::list<std::string>::iterator it2;
				int i = 0;
				it2 = lst2.begin();
				for (it1 = lst1.begin(); it1 != lst1.end(), it2 != lst2.end(); it1++) {
					if ((*it1) != (*it2)) {
						return false;
					}
					it2++;
			...
			if (typeid(*a) == typeid(LM::Transfer_node)) {
				...
				std::list<LM::transition_descriptor> lst1 = t_a->get_transitions_list();
				std::list<LM::transition_descriptor> lst2 = t_b->get_transitions_list();
				std::list<LM::transition_descriptor>::iterator it1;
				std::list<LM::transition_descriptor>::iterator it2;
				int i = 0;
				it2 = lst2.begin();
				for (it1 = lst1.begin(); it1 != lst1.end(), it2 != lst2.end(); it1++) {
					if (((*it1).line_name != (*it2).line_name)||(((*it1).station_name)!=((*it2).station_name))) {
						return false;
					}
					it2++;
				}
...
TEST(Crossing_station_class, empty_constructor) {
	...
	std::list<std::string> lst = c.get_transition_lines();
...
TEST(Transfer_node_class, empty_constructor) {
	...
	std::list<LM::transition_descriptor> lst = t.get_transitions_list();
	std::list<LM::transition_descriptor>::iterator it;
	for (it = lst.begin(); it != lst.end(); it++) {
		ASSERT_EQ((*it).line_name, "");
		ASSERT_EQ((*it).station_name, "");
	}
}
...
TEST(Transfer_node_class, inititialize_constructor2) {
	...
	int i = 0;
	for (it = lst.begin(); it != lst.end(); it++) {
		ASSERT_EQ((*it).line_name, mas[i].line_name);
		ASSERT_EQ((*it).station_name, mas[i].station_name);
		i++;
	}
}
...
TEST(Transfer_node_class, add_transition) {
	...
	int i = 0;
	for (it = lst.begin(); it != lst.end(); it++) {
		ASSERT_EQ((*it).line_name, check_mas[i].line_name);
		ASSERT_EQ((*it).station_name, check_mas[i].station_name);
		i++;
	}
}
...
TEST(Line_descriptor_class, insert) {
	...
	std::list<LM::Station*>::iterator lst_it;
	int i = 0;
	lst_it = line.iter_begin();
	EXPECT_TRUE(comparison(((*lst_it)->copy()), st1));
	lst_it++;
	EXPECT_TRUE(comparison(((*lst_it)->copy()), st2));
	lst_it++;
	EXPECT_TRUE(comparison(((*lst_it)->copy()), st3));
}
...
TEST(Line_descriptor_class, remove_station) {
	...
	std::list<LM::Station*>::iterator lst_it;
	int i = 0;
	for (lst_it = line.iter_begin(); lst_it != line.iter_end(); lst_it++) {
		EXPECT_TRUE(comparison(((*lst_it)->copy()), check_mas[i]));
		i++;
	}
}
